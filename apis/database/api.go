...
func AddUserCredential(loginName string,pwd string) error {
	statment,err := DbConnection.Prepare("INSERT INTO users(loginName,password) VALUES(?,?)")
	if err != nil{
		return err
	}
	_,err = statment.Exec(loginName,pwd)
	if err != nil{
		return err
	}
...
func AddNewVideo(aid int, name string) (*models.VideoInfo, error) {
	...
	stmtIns, err := DbConnection.Prepare(`INSERT INTO video_info 
...
func GetVideoInfo(vid string) (*models.VideoInfo, error) {
	stmtOut, err := DbConnection.Prepare("SELECT author_id, name, display_ctime FROM video_info WHERE id=?")
...
func AddNewComments(vid string, aid int, content string) error {
	...
	stmtIns, err := DbConnection.Prepare("INSERT INTO comments (id, video_id, author_id, content) values (?, ?, ?, ?)")
	if err != nil {
		return err
	}
...
func ListComments(vid string, from, to int) ([]*models.Comment, error) {
	stmtOut, err := DbConnection.Prepare(` SELECT comments.id, users.Login_name, comments.content FROM comments
